...
class InvoiceSignPage extends StatefulWidget {
  ...
  final int startKiloMeter;
  final int docLocation; // Added pdfId
  final int invoiceId;
  final int customerId;
  final int pdfId; // Added pdfId

  const InvoiceSignPage({
    Key? key,
    required this.pdfLink,
    required this.docType,
    required this.invoiceNum,
    required this.startKiloMeter,
    required this.pdfId,
    required this.invoiceId,
    required this.customerId,
    required this.docLocation,
    required this.signedPdfLink,
    required this.cusLatitude,
    required this.cusLongitude,
  }) : super(key: key);
  @override
  State<InvoiceSignPage> createState() => _InvoiceSignPageState();
}

class _InvoiceSignPageState extends State<InvoiceSignPage> {
  String? localPath;
  bool isLoading = true;
  ...
  String? _customerAddressConfirmed;
  ...
  Future<void> loadPDF() async {
    ...
    try {
      ...
      final file = File('${dir.path}/temp_invoice_${widget.invoiceNum}.pdf');
  ...
  Future<void> _submitSignature() async {
    ...
    try {
      ...
      final payloadData = {
        ...
        'latitude': latitude,
        'longitude': longitude,
        'customerAddressConfirmed': _customerAddressConfirmed,
      ...
      final message = await _driverService.uploadSignature(
      ...
      } else if (currentDepartmentId == 10) {
        Navigator.pushAndRemoveUntil(
          context,
  ...
  Widget build(BuildContext context) {
    ...
    final pdfUrl =
        widget.signedPdfLink?.isNotEmpty == true
            ...
            : widget.pdfLink;
    final docTitle = widget.invoiceNum ?? 'Invoice';
    return Scaffold(
      // appBar: Navbar(title: "Sign ${widget.invoiceNum}", backButton: true),
      appBar: AppBar(
        title: Tooltip(
          message: docTitle,
          child: Text(
            "Sign ${widget.invoiceNum}",
          ...
          if (pdfUrl != null &&
            ...
            PopupMenuButton<String>(
              ...
              onSelected: (value) async {
                ...
                final fileName = '${widget.invoiceNum}.pdf';
                final pdfType =
                    (widget.signedPdfLink?.isNotEmpty == true)
                        ? 'signed'
                        : 'unsigned';
                                  ...
                                  children: [
                                    ...
                                    ElevatedButton(
                                      onPressed: () {
                                        ...
                                        if (kIsWeb) {
                                          // For web, just rebuild the WebPdfViewer
                                  ...
                                  onPressed:
                                      _isSubmitting
                                          ...
                                          : () async {
                                            ...
                                            // Step 2: Check if both lat/lng are missing
                                            final bool isLocationMissing =
                                                (widget.cusLatitude == null ||
                                                    widget.cusLatitude!
                                                        .trim()
                                                        .isEmpty) &&
                                                (widget.cusLongitude == null ||
                                                    widget.cusLongitude!
                                                        .trim()
                                                        .isEmpty);
                                            if (isLocationMissing) {
                                              // Auto-confirm without popup
                                              setState(() {
                                                _customerAddressConfirmed =
                                                    "Yes";
                                              });
                                            }
